...
public class TreeCellEditor extends DefaultTreeCellEditor implements ActionListener {
	...
	public void actionPerformed(ActionEvent e){
		if (stavka instanceof Node){
				
				if(stavka instanceof SoftverskaKompanija) return;
				((Node) stavka).setName(e.getActionCommand());
				//MyTab.getInstance().getTabbedPane().remove(index);
				//MyTab.getInstance().getTabbedPane().addTab(((Thing) stavka).getName(), new JTextField(((Thing) stavka).getName()));
